...
Function ImageScanNItrEFM(xpos, ypos, liftheight, scansizeX,scansizeY, scanlines, scanpoints, scanspeed, numavgsperpoint, xoryscan, fitstarttime, fitstoptime)
...
//	endif
	...
	do
		...
		error+= td_xsetoutwavepair(0,"Event.2,repeat", "Output.B", voltagewave,"Output.C", triggerwave,-1*interpval)
...
//		SetFeedbackLoop(3, "always",  "ZSensor", ReadWaveZ[scanpoints-1]-liftheight*1e-9/GV("ZLVDTSens"),0,EFMFilters[%ZHeight][%IGain],0, "Output.Z",0) // note the integral gain of 10000
		...
		td_StopInWaveBank(-1)
		td_StopOutWaveBank(-1)
		td_wv("Output.C", 0)	// set trigger to 0
		td_wv("Output.A", 0)
		td_wv("Output.B", 0)
