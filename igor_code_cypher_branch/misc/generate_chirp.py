...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', dest='sampling_rate', help='Sampling Rate (Hz), def=10 MHz', required=False)

    argument = parser.parse_args()
    f_center = float(argument.freq_center)
    f_width = float(argument.freq_width)
    f_length = float(argument.length)

    if argument.sampling_rate:
        chirp = GenChirp(f_center, f_width, length=f_length, sampling_rate=argument.sampling_rate)
    else:
