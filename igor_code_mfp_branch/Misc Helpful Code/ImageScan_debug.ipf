...
Function ImageScantrEFM(xpos, ypos, liftheight, scansizeX,scansizeY, scanlines, scanpoints, scanspeed, numavgsperpoint, xoryscan, fitstarttime, fitstoptime)
	...
	// debugging
	Make/O/N=64 ZPositions, ZDifferences
	...
	do
		...
		// debugging
		error += td_xsetinwave(2, "Event.2", "ZSensor", ZPositions, "", -UpInterpolation)
		...
		// debugging
		Print "Height difference at Point 20 is = ", (ZPositions[20] - ReadWaveZ[63- 20]) * td_rv("ZLVDTSens")	
		Print "Height difference at Point 50 is = ", (ZPositions[50] - ReadWaveZ[63- 50]) * td_rv("ZLVDTSens")	
...
//		ReadWaveZmean = Mean(ReadwaveZ) * ZLVDTSens
//		Topography[][i] = -(ReadwaveZ[p] * ZLVDTSens-ReadWaveZmean)
		Topography[][i] = -(ReadwaveZ[p] * ZLVDTSens)
		DoUpdate
...
// debugging
		...
		for(counter = 0; counter < scanpoints;counter+=1)
			FrequencyOffset[counter][i] = shift_wave[scanpoints - counter - 1] * -1
		...
		if (i < scanlines)//////////////
			...
			td_wv("PIDSLoop.3.Setpoint", ReadWaveZ[0])
