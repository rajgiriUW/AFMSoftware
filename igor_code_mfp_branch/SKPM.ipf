...
// SKPM (Scanning Kelvin Probe Microscopy) acquisition and analysis functions.
// Covers standard FM-SKPM, voltage-pulse variants, intensity-modulated FM-SKPM,
// surface photovoltage (SPV) imaging, and IM-SKPM averaging utilities.
//
// --- Acquisition ---
//
// ImageScanSKPM(xpos, ypos, liftheight, scansizeX, scansizeY, scanlines,
//               scanpoints, scanspeed)
//   2D raster SKPM image. Calls PointScanSKPM per pixel and stores the
//   contact potential difference (CPD) map.
//
// PointScanSKPM(xpos, ypos, liftheight, dwelltime)
//   Single-point FM-SKPM. Lifts to liftheight, dwells for dwelltime,
//   and records CPD via lock-in nulling feedback.
//
// PointScanSKPMVoltagePulse(xpos, ypos, liftheight, dwelltime,
//                            appliedbias, biasfreq)
//   SKPM with a superimposed AC voltage pulse at biasfreq. Used for
//   transient CPD and charge injection measurements.
//
// PointScanSKPMVoltagePulseTest(xpos, ypos, liftheight, dwelltime,
//                                appliedbias, biasfreq)
//   Development/test variant of PointScanSKPMVoltagePulse.
//
// ImageScanSKPMSPV(xpos, ypos, liftheight, scansizeX, scansizeY,
//                  scanlines, scanpoints, scanspeed)
//   Surface photovoltage SKPM image. Interleaves dark/illuminated
//   measurements per pixel to extract the SPV map.
//
// --- UI Callbacks ---
//
// SKPMSPVImageScanButton(ctrlname)
//   Button handler that launches ImageScanSKPMSPV from the panel.
//
// UseLineNumforVoltage(cba)
//   Checkbox handler to toggle per-line voltage indexing in SPV scans.
